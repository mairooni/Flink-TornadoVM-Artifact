...
public abstract class DataSet<T> {
	...
	public <R> MapOperator<T, R> map(MapFunction<T, R> mapper) {
		...
		if (tornado || Utils.tornadoVM) {
			//long skeletonStart = System.currentTimeMillis();
			String mapUserClassName = mapper.getClass().toString().replace("class ", "").replace(".", "/");
			if (AcceleratedFlinkUserFunction.getTornadoMapBytes() == null) {
				AcceleratedFlinkUserFunction.transformUDF(mapUserClassName);
			} else if (AcceleratedFlinkUserFunction.getTornadoMap2Bytes() == null) {
				AcceleratedFlinkUserFunction.transformUDF2(mapUserClassName);
			} else if (AcceleratedFlinkUserFunction.getTornadoMap3Bytes() == null) {
				AcceleratedFlinkUserFunction.transformUDF3(mapUserClassName);
			} else if (AcceleratedFlinkUserFunction.getTornadoMap4Bytes() == null) {
				AcceleratedFlinkUserFunction.transformUDF4(mapUserClassName);
			} else if (AcceleratedFlinkUserFunction.getTornadoMap5Bytes() == null) {
				AcceleratedFlinkUserFunction.transformUDF5(mapUserClassName);
			} else {
				AcceleratedFlinkUserFunction.transformUDF(mapUserClassName);
			}
		//	long skeletonEnd = System.currentTimeMillis();
			//System.out.println("**** Function " + mapper + " create ASM Skeleton: " + (skeletonEnd - skeletonStart));
		}
	...
	public ReduceOperator<T> reduce(ReduceFunction<T> reducer) {
		...
		if ((tornado || Utils.tornadoVM) /*&& !reducer.getClass().toString().contains("UpdateAccumulator")*/) {
			//long skeletonStart = System.currentTimeMillis();
			String reduceUserClassName = reducer.getClass().toString().replace("class ", "").replace(".", "/");
			if (AcceleratedFlinkUserFunction.getTornadoReduceBytes() == null) {
				AcceleratedFlinkUserFunction.transformReduceUDF(reduceUserClassName);
			} else if (AcceleratedFlinkUserFunction.getTornadoReduceBytes2() == null) {
				AcceleratedFlinkUserFunction.transformReduceUDF2(reduceUserClassName);
			} else {
				AcceleratedFlinkUserFunction.transformReduceUDF(reduceUserClassName);
			}
			//long skeletonEnd = System.currentTimeMillis();
	//		System.out.println("**** Function " + reducer + " create ASM Skeleton: " + (skeletonEnd - skeletonStart));
		}
