...
import org.apache.flink.api.asm.AcceleratedFlinkUserFunction;
import org.apache.flink.api.asm.TornadoMap;
...
public class JobGraphGenerator implements Visitor<PlanNode> {
	...
	HashMap<String, TypeInformation[]> typeInfoHM = new HashMap<>();
...
//		Configuration dataSinkConf = null;
//		Configuration iterationTailConf = null;
//		Configuration iterativeDataSetConf = null;
//		boolean iterations = false;
//		String iterationTailName = null;
//		Configuration iterationTailCollectorConf = null;
//		String iterationTailCollectorName = null;
		boolean tornadoVMFlag = false;

		for (JobVertex jv : vertices.values()) {
			if (jv.getName().contains("tornado")) {
				tornadoVMFlag = true;
				break;
			}
		}
		//System.out.println("------------- TORNADO FLAG: " + tornadoVMFlag);
		if (tornadoVMFlag) {
			for (JobVertex jv : vertices.values()) {
				jv.setParallelism(1);
			}
		...
		if (tornadoVMFlag) {
			for (JobVertex jv : vertices.values()) {
				if (!jv.getName().contains("PartialSolution") && !jv.getName().contains("Sync")) {
					Configuration conf = jv.getConfiguration();
					conf.setTornadoVMFlag();
				}
			}
		}

		if (tornadoVMFlag || tornado) {
			for (JobVertex jv : vertices.values()) {
				if (!jv.getName().contains("PartialSolution") && !jv.getName().contains("Sync")) {
					Configuration conf = jv.getConfiguration();
					if (AcceleratedFlinkUserFunction.getTornadoMapBytes() != null) {
						conf.setSkeletonMapBytes(AcceleratedFlinkUserFunction.getTornadoMapBytes());
					}
					if (AcceleratedFlinkUserFunction.getTornadoMap2Bytes() != null) {
						conf.setSkeletonMapBytes2(AcceleratedFlinkUserFunction.getTornadoMap2Bytes());
					}
					if (AcceleratedFlinkUserFunction.getTornadoMap3Bytes() != null) {
						conf.setSkeletonMapBytes3(AcceleratedFlinkUserFunction.getTornadoMap3Bytes());
					}
					if (AcceleratedFlinkUserFunction.getTornadoMap4Bytes() != null) {
						conf.setSkeletonMapBytes4(AcceleratedFlinkUserFunction.getTornadoMap4Bytes());
					}
					if (AcceleratedFlinkUserFunction.getTornadoMap5Bytes() != null) {
						conf.setSkeletonMapBytes5(AcceleratedFlinkUserFunction.getTornadoMap5Bytes());
					}
					if (AcceleratedFlinkUserFunction.getTornadoReduceBytes() != null) {
						conf.setSkeletonReduceBytes(AcceleratedFlinkUserFunction.getTornadoReduceBytes());
					}
					if (AcceleratedFlinkUserFunction.getTornadoReduceBytes2() != null) {
						conf.setSkeletonReduceBytes2(AcceleratedFlinkUserFunction.getTornadoReduceBytes2());
					}
					ByteArrayOutputStream bObj = new ByteArrayOutputStream();
					ObjectOutputStream out;
					try {
						out = new ObjectOutputStream(bObj);
						out.writeObject(this.typeInfoHM);
						out.close();
						bObj.close();
						byte[] byteOut = bObj.toByteArray();
						conf.setTypeInfo(byteOut);
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
...
//		for (JobVertex jv : vertices.values()) {
//			if (jv.getName().contains("tornado")) tornadoVMFlag = true;
//			if (!jv.toString().contains("PartialSolution")) {
//
//				Configuration conf = jv.getConfiguration();
//
//				if (jv.toString().contains("DataSource") && jv.toString().contains("broadcasted")) {
//					conf.broadcastedDataSet();
//				}
//
//				if (jv.toString().contains("IterationTailTask")) {
//					iterationTailConf = conf;
//					String name = jv.getName();
//					jv.setName("tail " + name);
//					iterationTailName = jv.getName();
//				} else if (jv.toString().contains("DataSink")) {
//					dataSinkConf = conf;
//				}
//
//				//System.out.println("Node: " + jv.toString());
//				for (IntermediateDataSet intrm : jv.getProducedDataSets()) {
//					for (JobEdge consumer : intrm.getConsumers()) {
//						JobVertex inputConsumer = consumer.getTarget();
//						if (inputConsumer.toString().contains("PartialSolution")) {
//							iterations = true;
//							if (jv.toString().contains("DataSource")) {
//							//	System.out.println("DataSource conf: " + conf);
//								iterativeDataSetConf = conf;
//								conf.setAsIterationDataSet();
//							}
//							JobVertex psVertex = null;
//							for (JobVertex jvv : vertices.values()) {
//								if (jvv.toString().contains("PartialSolution")) {
//									psVertex = jvv;
//									break;
//								}
//							}
//							for (IntermediateDataSet intermPs : psVertex.getProducedDataSets()) {
//								for (JobEdge consumer2 : intermPs.getConsumers()) {
//									JobVertex consu = consumer2.getTarget();
//									if (!consu.toString().contains("Sync") && !consu.toString().contains("DataSink") && consu.toString().contains("Tail")) {
//										Configuration confSuc = consu.getConfiguration();
//										confSuc.addPredecessorConfig(conf);
//										confSuc.addPredecessorName(jv.getName());
//									}
//									if (!consu.toString().contains("Sync") && !consu.toString().contains("DataSink") && !consu.toString().contains("Tail")) {
//										iterationTailCollectorConf = consu.getConfiguration();
//										iterationTailCollectorName = consu.getName();
//									}
//								}
//							}
//						} else {
//							Configuration confSuc = inputConsumer.getConfiguration();
//							//System.out.println("\t---consumer: " + inputConsumer);
//							confSuc.addPredecessorConfig(conf);
//							confSuc.addPredecessorName(jv.getName());
//						}
//					}
...
//
//		if (iterations) {
//			iterativeDataSetConf.addIterationTailConfig(iterationTailConf);
//			if (iterationTailCollectorConf == null) {
//				dataSinkConf.addPredecessorConfig(iterationTailConf);
//				dataSinkConf.addPredecessorName(iterationTailName);
//			} else {
//				iterationTailCollectorConf.addPredecessorConfig(iterationTailConf);
//				iterationTailCollectorConf.addPredecessorName(iterationTailName);
//				iterationTailCollectorConf.isBroadcastedDataset();
//				//dataSinkConf.addPredecessorConfig(iterationTailCollectorConf);
//				//dataSinkConf.addPredecessorName(iterationTailCollectorName);
//			}
//		}
//
//		if (tornadoVMFlag) {
...
//		for (JobVertex jv : vertices.values()) {
//			System.out.println("Node: " + jv.toString());
//			System.out.println("\t -> conf: " + jv.getConfiguration());
//			int numOfPred = jv.getConfiguration().getNumberOfPredecessors();
//			if (numOfPred == 1) {
//				System.out.println("-> predName: " + jv.getConfiguration().getPredecessorName(0));
//				Configuration predConf = jv.getConfiguration().getPredecessorConfig(0);
//				System.out.println("\t -> predConf: " + predConf);
//			} else {
//				System.out.println("-> predName1: " + jv.getConfiguration().getPredecessorName(0));
//				Configuration predConf1 = jv.getConfiguration().getPredecessorConfig(0);
//				System.out.println("\t -> predConf1: " + predConf1);
//				System.out.println("-> predName2: " + jv.getConfiguration().getPredecessorName(1));
//				Configuration predConf2 = jv.getConfiguration().getPredecessorConfig(1);
//				System.out.println("\t -> predConf2: " + predConf2);
//			}
//		}
		...
		graph.setTypeInfo(this.typeInfoHM);
		try {
	...
	public boolean preVisit(PlanNode node) {
		...
		try {
			if (node instanceof SinkPlanNode) {
				...
				OptimizerNode on = node.getOptimizerNode();
				//SingleInputPlanNode sn = ((SingleInputPlanNode) node).template;
				Operator op = on.getOperator();
				UnaryOperatorInformation info = (UnaryOperatorInformation) op.getOperatorInfo();
				vertex = createSingleInputVertex((SingleInputPlanNode) node);
				TypeInformation[] typeInfo = new TypeInformation[2];
				typeInfo[0] = info.getInputType();
				typeInfo[1] = info.getOutputType();
				this.typeInfoHM.put(op.getUserCodeWrapper().getUserCodeObject().toString(), typeInfo);
			}
