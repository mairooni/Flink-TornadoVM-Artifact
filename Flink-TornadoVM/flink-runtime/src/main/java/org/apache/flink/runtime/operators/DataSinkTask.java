...
public class DataSinkTask<IT> extends AbstractInvokable {
	...
	private int deviceIndex = Integer.parseInt(System.getProperty("tornado.device", "0"));
	...
	public void invoke() throws Exception {
		...
		try {
			...
			//if (AbstractInvokable.kmeans) {
			//	TypeInformation<?> type = TypeExtractor.getForObject(input.next());
			//	AbstractInvokable.returnType = type;
			//}
			boolean tornadoConfFlag = config.getConfiguration().getTornadoVMFlag();
			if (false) {
				//System.out.println("%%%%%%% DATASINK: " + tornadoConfFlag);
				Configuration predConf = config.getConfiguration().getPredecessorConfig(0);
				//System.out.println("Datasink predconf: " + predConf);
				AccelerationData acdata = (AccelerationData) predConf.getAccelerationData();
				int recordSize = acdata.getReturnSize();
				byte[] results = acdata.getRawData();
//				System.out.println("DataSink Input");
//				for (int i = 0; i < results.length; i++) {
//					System.out.print(results[i] + " ");
//				}
//				System.out.println("\n");
				boolean diffReturnSize = acdata.isDifferentReturnTupleFields();
				ArrayList<Integer> tupleReturnSizes = acdata.getReturnFieldSizes();
				// work!
				//TODO: Add extra header bytes to array to reconstruct it as the serializer would
				long retStart = System.currentTimeMillis();
				Utils.CollectHelper h = (Utils.CollectHelper) format;
				SerializedListAccumulator s = h.accumulator;
				int numOfBytes = 0;

				boolean arrayField = false;
				int lengthOfArrayField = 0;
				int arrayFieldNo = 0;
				int totalBytes = 0;
				if (acdata.getArrayField()) {
					arrayField = true;
					lengthOfArrayField = acdata.getLengthOfArrayField();
					arrayFieldNo = acdata.getArrayFieldNo();
					totalBytes = acdata.getTotalBytes();
					recordSize = acdata.getRecordSize();
				}

				if (tupleReturnSizes != null) {
					results = changeEndianess(arrayField, results, recordSize, diffReturnSize, tupleReturnSizes);
				} else {
					if (results.length == 4) {
						changeOutputEndianess4(results, 0);
					} else if (results.length == 8) {
						changeOutputEndianess8(results, 0);
					}
				}

				byte[] recordB;
				while (!this.taskCanceled && (numOfBytes < results.length)) {
					if (arrayField) {
						recordB = getReturnRecordArrayField(results, numOfBytes, tupleReturnSizes,lengthOfArrayField,arrayFieldNo, totalBytes, diffReturnSize);
					} else if (diffReturnSize) {
						recordB = getReturnRecordDiffTypes(results, numOfBytes, tupleReturnSizes);
					} else {
						recordB = Arrays.copyOfRange(results, numOfBytes, (numOfBytes + recordSize));
					}
					//recordB = Arrays.copyOf(recordB, 32);

					s.tornadoResults(recordB);
					numOfBytes += recordSize;
				}
				long retEnd = System.currentTimeMillis();
				//System.out.println("### Write byte results to accumulator: " + (retEnd - retStart));
			} else {
	...
	public byte[] getReturnRecordDiffTypes(byte[] results, int from, ArrayList<Integer> tupleReturnSizes) {
		int numOfFields = tupleReturnSizes.size();
		int fieldSizes = 0;
		for (Integer size : tupleReturnSizes) {
			fieldSizes += size;
		}

		byte[] record = new byte[numOfFields * fieldSizes];

		int k = 0;
		for (int i = 0; i < numOfFields; i++) {
			if (tupleReturnSizes.get(i) == 4) {
				int start = from + 4 + (8 * i);
		...
		if (arrayField) {
			if (diffReturnSize) {
				for (int i = 0; i < b.length; i += 8) {
					changeOutputEndianess8(b, i);
				}
			} else {
				for (int i = 0; i < b.length; i += 4) {
					changeOutputEndianess4(b, i);
				}
			}
			return b;
		}
		if (diffReturnSize) {
			for (int i = 0; i < b.length; i += recordSize) {
				for (int j = 0; j < tupleReturnSizes.size(); j++) {
					changeOutputEndianess8(b, off + i);
					off += 8;
				}
				off = 0;
			}
			...
			for (int i = 0; i < b.length; i += recordSize) {
				for (int j = 0; j < tupleReturnSizes.size(); j++) {
					if (tupleReturnSizes.get(j) == 4) {
						changeOutputEndianess4(b, off + i);
						off += 4;
	...
	public static void changeOutputEndianess4(byte[] output, int i) {
		byte tmp;
		// swap 0 and 3
		tmp = output[i];
		output[i] = output[i + 3];
	...
	public static void changeOutputEndianess8(byte[] output, int i) {
		byte tmp;
		// swap 0 and 7
		tmp = output[i];
		output[i] = output[i + 7];
