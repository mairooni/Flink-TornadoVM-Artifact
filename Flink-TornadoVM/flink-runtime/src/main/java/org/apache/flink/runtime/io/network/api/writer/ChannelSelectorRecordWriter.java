...
public final class ChannelSelectorRecordWriter<T extends IOReadableWritable> extends RecordWriter<T> {
	...
	public void emit(byte[] b) {
		try {
			if (serializer instanceof SpanningRecordSerializer) {
				((SpanningRecordSerializer) serializer).writeByteArray(b);
			}

			// Make sure we don't hold onto the large intermediate serialization buffer for too long
			if (copyFromSerializerToTargetChannel(0)) {
				serializer.prune();
			}
		} catch (Exception e) {
			System.out.println("Exception in ChannelSelectorRecordWriter");
		}
	}
