...
public class CountingCollector<OUT> implements Collector<OUT> {
	...
	public final Counter numRecordsOut;
	...
	public void collect(byte[] b) {
		...
		if (this.collector instanceof OutputCollector) {
			((OutputCollector) collector).collect(b);
		} else if (this.collector instanceof WorksetUpdateOutputCollector) {
			((WorksetUpdateOutputCollector) collector).collect(b);
		}
	}
