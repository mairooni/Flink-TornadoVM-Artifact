...
public class DataSourceTask<OT> extends AbstractInvokable {
	...
	private static final boolean DEBUG_TORNADO_FLINK = false;
	...
	// byte buffer containing raw data
			...
			while (!this.taskCanceled && splitIterator.hasNext())
			{
				...
				try {
					...
					if (objectReuseEnabled) {
						...
						boolean tornadoConfFlag = config.getConfiguration().getTornadoVMFlag();
						if ((tornado || tornadoConfFlag) && (this.output instanceof ChainedAllReduceDriver || this.output instanceof ChainedMapDriver)) {
							if (format instanceof TupleCsvInputFormat) {
								DataTransformation dtrans = new DataTransformation();
								AccelerationData acdata = dtrans.generateInputAccelerationData(inputDataBytes, serializer, null, false, false, true, false);
								if (this.output instanceof ChainedAllReduceDriver) {
									ChainedAllReduceDriver chred = (ChainedAllReduceDriver) this.output;
									chred.collect(acdata);
								} else if (this.output instanceof ChainedMapDriver) {
									ChainedMapDriver chmad = (ChainedMapDriver) this.output;
									chmad.collect(acdata);
								}
							} else {
								if (this.output instanceof ChainedAllReduceDriver) {
									ChainedAllReduceDriver chred = (ChainedAllReduceDriver) this.output;
									DataTransformation dtrans = new DataTransformation();
									// TODO: Support cases where input format is not of type CollectionInputFormat
									CollectionInputFormat cif = (CollectionInputFormat) format;
									ArrayList a = (ArrayList) cif.getDataSet();
									AccelerationData acdata = dtrans.generateInputAccelerationData(inputDataBytes, serializer, a, true, false, true, false);
									chred.collect(acdata);
								} else if (this.output instanceof ChainedMapDriver) {
									ChainedMapDriver chmad = (ChainedMapDriver) this.output;
									DataTransformation dtrans = new DataTransformation();
	...
	private void initInputFormatTornado() {
		...
		inputDataBytes = taskConf.getBytes("udf", null);
	}
