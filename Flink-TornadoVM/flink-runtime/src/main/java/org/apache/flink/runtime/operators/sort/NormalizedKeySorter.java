...
public final class NormalizedKeySorter<T> implements InMemorySorter<T> {
			...
			List<MemorySegment> memory, int maxNormalizedKeyBytes)
	{
		...
		if (comparator instanceof TupleComparator) {
			// we currently support one key
			this.tupleKey = ((TupleComparator) comparator).keyPositions[0];
		}
	...
	private byte[] readBytesFromBuffer (long pointer, int recordSize) {
		try {
			this.recordBuffer.setReadPosition(pointer);
			byte[] bytes = new byte[recordSize];
			this.recordBuffer.read(bytes, 0, recordSize);
			return bytes;
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public class NormalizedKeySorterIterator implements MutableObjectIterator<T> {
		...
		public byte[] getBytes(int recordSize) {
			byte[] total = null;
			ArrayList<byte[]> byteRecords = new ArrayList<>();
			while (this.current < this.size) {
				...
				long pointer = this.currentIndexSegment.getLong(this.currentOffset);
				this.currentOffset += indexEntrySize;
				byte[] b =  readBytesFromBuffer(pointer, recordSize);
				byteRecords.add(b);
			}
			int totalSize = 0;
			...
			for (byte[] record : byteRecords) {
				if (total == null) {
					total = Arrays.copyOf(record, totalSize);
					destPos += record.length;
				} else {
					System.arraycopy(record, 0, total, destPos, record.length);
					destPos += record.length;
				}
			}
			return total;
	...
	public final MutableObjectIterator<T> getIterator() {
		return new NormalizedKeySorterIterator();
	}
