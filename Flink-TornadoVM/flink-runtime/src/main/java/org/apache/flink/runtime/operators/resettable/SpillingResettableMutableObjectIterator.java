...
public class SpillingResettableMutableObjectIterator<T> implements ResettableMutableObjectIterator<T> {
	...
	public final SpillingBuffer buffer;
	...
	public byte[] getDataInBytes() throws IOException {
		if (this.inView == null) {
			if (input instanceof ReaderIterator) {
				byte[] data = ((ReaderIterator) input).getDataInBytes();
				return data;
			} else {
				return null;
			}
		} else {
			if (this.inView instanceof RandomAccessInputView) {
				byte[] b = new byte[tornadoDataSize];
				try {
					this.inView.read(b, 0, b.length);
					return b;
				} catch (IOException e) {
					e.printStackTrace();
				}
