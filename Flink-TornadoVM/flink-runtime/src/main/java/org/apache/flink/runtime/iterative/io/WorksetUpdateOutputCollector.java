...
public class WorksetUpdateOutputCollector<T> implements Collector<T> {
	...
	public void collect (byte[] byteRecord) {
		if (this.serializer instanceof TupleSerializer) {
			((TupleSerializer) this.serializer).writeBytesInMemory(byteRecord, this.outputView);
	...
	public void collect(T record) {
		// if we are running Flink-Tornado, the record value will be null in order to "fake" execution
		if (record == null || tornado) {
			if (record == null) System.out.println("RECORD NULL: IF RUNNING WITH TORNADOVM IGNORE THIS MESSAGE");
			return;
		}
		try {
