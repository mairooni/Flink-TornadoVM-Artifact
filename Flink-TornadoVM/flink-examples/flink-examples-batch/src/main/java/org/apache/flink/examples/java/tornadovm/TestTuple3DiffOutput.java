...
/**
 * Test Tuple3 return type, where fields are of different type.
 */
public class TestTuple3DiffOutput {
	...
	private static void testVectorAddFlinkTornado(String[] args, final int size) throws Exception {
		...
		DataSet<Tuple3<Integer, Integer, Double>> outArray = array.map(new VectorAdd());
		List<Tuple3<Integer, Integer, Double>> collect = outArray.collect();

		VectorAdd vadd = new VectorAdd();
		ArrayList<Tuple2<Integer, Integer>> serialArray = buildTuplesOrderSerial(size);
		List<Tuple3<Integer, Integer, Double>> serial = new ArrayList<>();
	...
	public static void main(String[] args) {
		...
		System.out.println("\033[1;36m Testing: org.apache.flink.examples.java.tornadovm.Test01#TestTuple3DiffOutput \033[0;0m");
		try {
			testVectorAdd(args);
	 ...
	 * Simple vector add.
	 */
	public static final class VectorAdd implements MapFunction<Tuple2<Integer, Integer>, Tuple3<Integer, Integer, Double>> {
		@Override
		public Tuple3<Integer, Integer, Double> map(Tuple2<Integer, Integer> value) {
			return new Tuple3<>(value.f0, value.f0 + value.f1, 2.2);
		}
	}
}
