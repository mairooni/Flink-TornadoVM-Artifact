...
/**
 * Test map function with signature Tuple3 map(Tuple3).
 */
public class TestTuple3Tuple3 {
	...
	private static DataSet<Tuple3<Integer, Integer, Integer>> buildTuplesOrder(ExecutionEnvironment env, final int size) {
		ArrayList<Tuple3<Integer, Integer, Integer>> arrayList = new ArrayList<>();
		for (int i = 0; i < size; i++) {
			Tuple3<Integer, Integer, Integer> t2 = new Tuple3<>(i, i, i);
			arrayList.add(t2);
		}
		return env.fromCollection(arrayList);
	}

	private static ArrayList<Tuple3<Integer, Integer, Integer>> buildTuplesOrderSerial(final int size) {
		ArrayList<Tuple3<Integer, Integer, Integer>> arrayList = new ArrayList<>();
		for (int i = 0; i < size; i++) {
			Tuple3<Integer, Integer, Integer> t2 = new Tuple3<>(i, i, i);
			arrayList.add(t2);
	...
	private static void testVectorAddFlinkTornado(String[] args, final int size) throws Exception {
		...
		DataSet<Tuple3<Integer, Integer, Integer>> array = buildTuplesOrder(env, size);
		...
		ArrayList<Tuple3<Integer, Integer, Integer>> serialArray = buildTuplesOrderSerial(size);
		List<Tuple3<Integer, Integer, Integer>> serial = new ArrayList<>();

		for (Tuple3<Integer, Integer, Integer> t : serialArray) {
	...
	public static void main(String[] args) {
		...
		System.out.println("\033[1;36m Testing: org.apache.flink.examples.java.tornadovm.TestTuple3Tuple3 \033[0;0m");
		try {
			testVectorAdd(args);
	 ...
	 * Simple vector add.
	 */
	public static final class VectorAdd implements MapFunction<Tuple3<Integer, Integer, Integer>, Tuple3<Integer, Integer, Integer>> {
		@Override
		public Tuple3<Integer, Integer, Integer> map(Tuple3<Integer, Integer, Integer> value) {
			return new Tuple3<>(value.f0, value.f0 + value.f1, value.f2);
		}
	}
}
