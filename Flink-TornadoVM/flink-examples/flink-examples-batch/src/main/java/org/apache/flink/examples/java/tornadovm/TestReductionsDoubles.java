...
public class TestReductionsDoubles {
	...
	private static Double[] buildDataSetDouble(final int size) {
	...
	private static void testRedFlinkTornado(String[] args, final int size) throws Exception {
		...
		Double[] input = buildDataSetDouble(size);
		DataSet<Double> array =  env.fromElements(input);
		DataSet<Double> outArray = array.reduce(new AddDoubles());
		List<Double> collect = outArray.collect();

		Double[] ser = new Double[1];
		ser[0] = 0.0;
		AddDoubles ad = new AddDoubles();
	...
	private static void testRedDoubles(String[] args) throws Exception {
	...
	public static void main(String[] args) {
		System.out.println("\033[1;36mTesting: org.apache.flink.examples.java.tornadovm.TestReductionsDoubles \033[0;0m");
		try {
			testRedDoubles(args);
	 ...
	 * A simple reduction.
	 */
	public static final class AddDoubles implements ReduceFunction<Double> {
		@Override
