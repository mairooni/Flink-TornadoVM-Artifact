...
/**
 *  A map with input Tuple3 and return type Double.
 */
public class TestTuple3Double {
	...
	private static Tuple3<Double, Double, Double>[] buildDataSetDouble(final int size) {
		Tuple3<Double, Double, Double>[] array = new Tuple3[size];
		Double d = 2.2;
		//Long l = 1L;
		for (int i = 0; i < array.length; i++) {
			array[i] = new Tuple3<>(d, d, d);
		}
		return array;
	}
	...
	private static void testTuple3FlinkTornado(String[] args, final int size) throws Exception {
		...
		Tuple3<Double, Double, Double>[] input = buildDataSetDouble(size);
		DataSet<Tuple3<Double, Double, Double>> array =  env.fromElements(input);
	...
	public static void main(String[] args) {
		System.out.println("\033[1;36mTesting: org.apache.flink.examples.java.tornadovm.TestTuple3Double \033[0;0m");
		try {
			testMapTuple3(args);
	 ...
	 * A simple reduction.
	 */
	public static final class MapTuple3 implements MapFunction<Tuple3<Double, Double, Double>, Double> {
		@Override
		public Double map(Tuple3<Double, Double, Double> value) {
