...
public class LogisticRegression {
	...
	//	final_paramsT.collect();

		DataSet<Tuple4<Double, Double, Double, Double>> results = test_data
			.map(new Predict()).withBroadcastSet(final_paramsT, "parameter")
			.reduce(new Evaluate())
			.map(new ComputeMetrics());
//		results.output(new DiscardingOutputFormat<>());
//		System.out.println(env.getExecutionPlan());
	...
	public static class Predict extends RichMapFunction<Tuple2<double[], Double>, Tuple4<Integer, Integer, Integer, Integer>> {
		...
		@Override
		public void open(Configuration parameters) {
			this.parameters = getRuntimeContext().getBroadcastVariable("parameter");
		}
...
//	}
	 ...
	 */
	public static class UpdateAccumulatorMap implements MapFunction<Tuple2<Tuple2<double[], Integer>, Integer>, Tuple2<Tuple2<double[], Integer>, Integer>> {
