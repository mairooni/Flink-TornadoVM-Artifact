...
/**
 *  A map with input Tuple3 and return type Integer.
 */
public class TestTuple3Integer {
	...
	private static Tuple3<Integer, Integer, Integer>[] buildDataSetDouble(final int size) {
		Tuple3<Integer, Integer, Integer>[] array = new Tuple3[size];
		for (int i = 0; i < array.length; i++) {
			array[i] = new Tuple3<>(i, i, i);
		}
		return array;
	}
	...
	private static void testTuple3FlinkTornado(String[] args, final int size) throws Exception {
		...
		Tuple3<Integer, Integer, Integer>[] input = buildDataSetDouble(size);
		DataSet<Tuple3<Integer, Integer, Integer>> array =  env.fromElements(input);
		DataSet<Integer> outArray = array.map(new MapTuple3());
	...
	public static void main(String[] args) {
		System.out.println("\033[1;36mTesting: org.apache.flink.examples.java.tornadovm.TestTuple3Integer \033[0;0m");
		try {
			testMapTuple3(args);
	 ...
	 * A simple reduction.
	 */
	public static final class MapTuple3 implements MapFunction<Tuple3<Integer, Integer, Integer>, Integer> {
		@Override
		public Integer map(Tuple3<Integer, Integer, Integer> value) {
